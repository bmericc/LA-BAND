...
if(!class_exists('FullwidthAudioPlayer')) {
	class FullwidthAudioPlayer {
		...
		public function __construct() {
            ...
            $this->fap_url = get_template_directory_uri() . '/fullwidth-audio-player';
		...
		public function setup_fap() {
			...
			$pp_args = array(
			  ...
			  'has_archive' => true, 
			  'hierarchical' => false,
			  'menu_icon' => $this->fap_url . '/admin/images/menu_icon.png',
		...
		public function enqueue_admin_styles_scripts( $hook ) {
			...
			if($hook == 'track_page_fap-options') {
				wp_enqueue_style( 'spectrum-colorpicker', $this->fap_url . "/admin/css/spectrum.css");
				wp_enqueue_style( 'fap-options', $this->fap_url . "/admin/css/options.css", array('thickbox') );
				
				wp_enqueue_script( 'spectrum-colorpicker', $this->fap_url . " /admin/js/spectrum.js" );
				wp_enqueue_script( 'fap-options', $this->fap_url . " /admin/js/options.js", array('media-upload', 'thickbox') );
			}
			else if($hook == 'track_page_fap-mp3-directory') {
				wp_enqueue_style( 'fap-mp3-directory', $this->fap_url . "/admin/css/mp3-directory.css" );
		...
		public function add_meta_boxes() {
			...
			wp_enqueue_script('fap-admin-new-track', $this->fap_url . '/admin/js/new-track.js');
		...
		public function add_custom_box() {
			...
			wp_enqueue_script('fap-admin-metabox', $this->fap_url . '/admin/js/metabox.js');
		...
		public function create_tracklists_meta_box() {
			...
			<div id="fap-single-track-form" class="hidden fap-setting">
      		...
      		<div class="fap-setting">
	          	...
	          	<img src="<?php echo $this->fap_url . '/admin/images/list.png'; ?>" alt="List Icon" />
	          	<input type="radio" name="fap_layout" value="grid" style="margin: 0 5px 0 10px;" />
	          	<img src="<?php echo $this->fap_url . '/admin/images/grid.png'; ?>" alt="Grid Icon" />
	          	<input type="radio" name="fap_layout" value="simple" style="margin: 0 5px 0 10px;" />
	          	<img src="<?php echo $this->fap_url . '/admin/images/simple_list.png'; ?>" alt="Simple List Icon" />
		...
		public function perform_filtering( $query ) {
		    $qv = &$query->query_vars;
		    if ( ( isset($qv['dt_playlist']) ) && is_numeric( $qv['dt_playlist'] ) ) {
		...
		public function add_scripts_styles() {
			...
			wp_enqueue_style( 'fullwidth-audio-player-tracks', $this->fap_url . '/css/fullwidthAudioPlayer-tracks.css' );
			wp_enqueue_style( 'fullwidth-audio-player', $this->fap_url . '/css/jquery.fullwidthAudioPlayer.css' );
			
			if($this->int_to_bool($audio_player_options['responsive_layout']))
				wp_enqueue_style( 'fullwidth-audio-player-responsive', $this->fap_url . '/css/jquery.fullwidthAudioPlayer-responsive.css' );
				
				
			if($this->int_to_bool($audio_player_options['store_playlist']))
				wp_enqueue_script( 'amplify-js', $this->fap_url . '/js/amplify.min.js', array(), '1.1.0' );
			
			wp_enqueue_script( 'soundcloud-sdk', 'https://connect.soundcloud.com/sdk.js' );
			wp_enqueue_script( 'fullwidth-audio-player', $this->fap_url . '/js/jquery.fullwidthAudioPlayer.js', array('jquery', 'jquery-ui-draggable', 'jquery-ui-sortable'), '1.5' );
				...
				<script type="text/javascript">
					...
					setTimeout(function() {
						...
						jQuery('#fullwidthAudioPlayer').fullwidthAudioPlayer({
							...
							popupUrl: '<?php echo $this->fap_url . 'popup.html';  ?>',
			...
			if( sizeof($files) ) {
				foreach($files as $file ) {
					...
					if($ThisFileInfo['error'] == null) {
						...
						$mp3_file['cover'] = isset($getID3->info['id3v2']['PIC'][0]['data']) || isset($getID3->info['id3v2']['APIC'][0]['data']) ? $this->fap_url . '/inc/cover.php?mp3_path='.$filepath.'' : null;
